...
const Dashboard = ({ role = "candidate" }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#001a26] text-gray-200">
        ...
        <nav className="flex flex-col gap-1 px-2">
          {menus[role].map(({ key, label, icon }) => (
            <button
              key={key}
              onClick={() => setActive(key)}
              className={`flex items-center gap-3 px-4 py-2 rounded-md text-sm hover:bg-white/10 transition ${
                active === key ? "bg-white/10" : ""
              }`}
            >
              {icon}
      ...
      {/* Mobile Navbar */}
      <nav className="md:hidden flex justify-around items-center bg-white/5 border-t border-white/10 py-2 fixed bottom-0 w-full z-[9999]">
        {menus[role].map(({ key, icon }) => (
          <button
            key={key}
            onClick={() => setActive(key)}
            className={`text-xl p-2 rounded-full ${
              active === key ? "bg-white/10 text-white" : "text-gray-400"
            }`}
          >
            {icon}
          </button>
